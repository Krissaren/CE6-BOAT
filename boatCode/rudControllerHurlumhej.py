...
def bearController(bear, refbear):   
    ...
    if len(bearList) < 10:
        ...
        if meanError < 0: #finds direction
            direction = -1
        ...
        nextBear = abs(Kp * (meanError - meanPreerror) +  Ki * delay * meanError + (Kd / delay) * (meanError - 2 * meanPreerror + meanPre2error)) + bearList[-1]
        nextBear = direction * nextBear
        #nextBear = Kp * (error)
